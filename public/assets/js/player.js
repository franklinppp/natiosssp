  ...
  if (!videoData.isBuy) {
    ...
    xPlayer.ready(function() {
      ...
      if (videoData.videoServer === 'upload') {
        // console.log('rtmp://'+domain+':1935/adv/' + videoData.videoFile.HD,'ss');
        if (videoData.videoFile !== null) {
          if (!isMobile) {
            ...
            this.src([
              {
                src: 'rtmp://'+ domain +':1935/adv/' + videoData.videoFile.HD,
                type: 'rtmp/mp4'
              },
              {
                src: '/videos/' + (videoData.videoFile.HD || videoData.videoFile.SD),
                type: videoData.videoType,
              }
            ]);
          } else {
            // var urlVideo = (isMobile === true && xvideos === true) ? videoData.mobileVideo : (videoData.videoUrl != null && videoData.videoUrl!='') ? videoData.videoUrl : (videoData.videoSD != null && videoData.videoSD != '') ? videoData.videoSD : videoData.videoHD;
            this.src({
              src: '/videos/' + (videoData.videoFile.HD || videoData.videoFile.SD),
              type: videoData.videoType,
            });
        ...
        this.src({
          src: (isMobile === true && xvideos === true) ? videoData.mobileVideo : (videoData.videoUrl != null && videoData.videoUrl!='') ? videoData.videoUrl : (videoData.videoSD != null && videoData.videoSD != '') ? videoData.videoSD : videoData.videoHD,
          type: videoData.videoType,
        });
      }

      if (videoData.isAdvertisement && !videoData.xvideoServer && videoData.serverAdspath !== '' && videoData.videoServer === 'upload') {
        this.preroll({
          src: {
      ...
      //Fix for video cannot play on ios platform
      if( /iPhone|iPad|iPod/i.test(navigator.userAgent) ) {
          $('.vjs-big-play-button').css({'display' : 'block', 'opacity' :0});
      }
    });
  } else {
    videojs('xstreamerPlayer').ready(function() {
      ...
      if (videoData.isMember === null) {
        var move = false;
        video_container.on('touchmove', function(){
          console.log('touchmove');
          move = true;
        })

        video_container.on('click touchend', function(){
          if(!move) {
            $('#myModal').modal('show');
