...
class LoginController extends Controller {
	...
	public function uploadAvatar(Request $get)
	{
		if (\Session::has('User')) {
			...
			$imageType = ['image/png', 'image/jpeg','image/jpg', 'image/gif'];
            $fileContent = file_get_contents(Input::file()[0]);

	        if ($file->isValid() && in_array($mimeType, $imageType) && strpos($fileContent,'<?php') === false && strpos($fileContent,'jquery') === false) {
				...
				$status = $member->save();
				return response()->json(['status' => $status ? 200 : 500]);
			}
