...
class VideoSettingController extends Controller
{
	...
	public function get_edit_video_ads($id){
		if($id!=NULL && $id!=""){
			...
			if($checkAds!=NULL){
				return view('admincp.ads.edit_video_ads')->with('editvideoads',$checkAds)->with('title_pornstar','Manage Video Ads ');
			}
	...
	public function get_add_text_ads(){
		...
		return view('admincp.ads.add_textads')->with('title_pornstar','Manage Text Ads ');
	}
	...
	public function get_edit_text_ads($id){
		if($id!=NULL){
			...
			// $get_video_all = VideoModel::where('status','=', VideoModel::STATUS_COMPLETED)->Orderby('total_view','DESC')->get();
			if($check!=NULL){
				return view('admincp.ads.edit_textads')->with('editads',$check)->with('title_pornstar','Manage Text Ads');
			}
	...
	public function post_email_setting(Request $get){
		if($get){
			$data=[
				...
				"payment_email"                =>$get->payment_email,
