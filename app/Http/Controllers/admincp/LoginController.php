...
class LoginController extends Controller {
	...
	public function post_option(Request $get) {
		if ($get) {
			...
			$link1 = isset($get->link1) ? $get->link1 : '';
			$link2 = isset($get->link2) ? $get->link2 : '';
			...
			$banner1 = Input::file('banner1');
			$banner2 = Input::file('banner2');
			...
			if ($banner1) {
				$validator = Validator::make(['banner1' => $banner1], ['banner1' => 'mimes:jpeg,png,gif,jpg',]);
				if ($validator->fails()) {
					return redirect('admincp/option')->with('msgerro', $validator->errors()->all()[0]);
				}

				$extension = $banner1->getClientOriginalExtension();

				$Allowed = ["png", "jpg", "gif", "PNG", "JPG", "GIF"];

				$destinationPath = public_path("upload/banner");

				$banner1File = $banner1->getClientOriginalName();
				if (in_array($extension, $Allowed)) {
					$banner1->move($destinationPath, $banner1File);
					$data["banner1"] = $banner1File;
				}
			}
			if ($banner2) {
				$validator = Validator::make(['banner2' => $banner2], ['banner2' => 'mimes:jpeg,png,gif,jpg',]);
				if ($validator->fails()) {
					return redirect('admincp/option')->with('msgerro', $validator->errors()->all()[0]);
				}

				$extension = $banner2->getClientOriginalExtension();

				$Allowed = ["png", "jpg", "gif", "PNG", "JPG", "GIF"];

				$destinationPath = public_path("upload/banner");

				$banner2File = $banner2->getClientOriginalName();
				if (in_array($extension, $Allowed)) {
					$banner2->move($destinationPath, $banner2File);
					$data["banner2"] = $banner2File;
				}
			}
	...
	public function post_payment_setting(Request $get) {
		if ($get) {
			...
			if ($update) {
				AppHelper::clearPaymentConfigCache();
	...
	public function post_edit_header_link(Request $get, $id) {
		if ($get) {
			...
			$update = HeaderModel::find($id)->update($data);
			if ($update) {
	...
	public function get_delete_header_link($id) {
		if ($id != NULL) {
			$checkid = HeaderModel::find($id);
			if ($checkid != NULL) {
				$delete = HeaderModel::find($id)->delete();
				if ($delete) {
	...
	public function post_add_tag(Request $get) {
		if ($get) {
			...
			$content_page = $get->content_page;
			if ($tag == NULL or $tag == "") {
				return redirect('admincp/add-tag')->with('msgerro', ' * Invalid Tag only text  format!')->with('tag', $tag)->with('content_page', $content_page);
			}
			$check = TagModel::where('tag', $tag)->first();
			if(empty($check)){
				$addtag = new TagModel();
				$addtag->tag = $tag;
				$addtag->status = 1;
			...
			return redirect('admincp/add-tag')->with('msgerro', 'Tag name exist. Please add other tag name')->with('tag', $tag)->with('content_page', $content_page);
		}
	}
