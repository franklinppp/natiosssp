...
class PornStarController extends Controller
{
	...
	public function post_editpornstar(Request $get, $id){
		$rules= [
			...
			'wall_poster' => 'image|mimes:jpeg,png,jpg',
		];
		...
		if(!empty($pornstar)) {
			$pornstar->title_name  =  $_POST["title_name"];
			$pornstar->post_name   =  str_slug($_POST["title_name"],"-");
			$pornstar->description  =  $_POST["description"];
			$pornstar->subscribe_status  =  isset($_POST["subscribe_status"]) ? 1 : 0 ;
			$pornstar->status  = $_POST["status"];
			$pornstar->gender = $_POST["gender"];
			$pornstar->age = $_POST['age'];
			$pornstar->born = $_POST['born'];
			$pornstar->height = $_POST['height'];
			$pornstar->ethnicity = $_POST['ethnicity'];
			$pornstar->hair_color = $_POST['hair_color'];
			$pornstar->eye_color = $_POST['eye_color'];
			...
			if($file) {
				...
				$filename = "Pornstar_Poster_".$id.'_'.str_slug($_POST["title_name"],'_').'_'.uniqid().".".$extension;

				if(!in_array($extension,$notAllowed)) {
						$file->move($destinationPath, $filename);
						$pornstar->poster = $filename;
			...
			if($wall) {
				...
				$filename = "Pornstar_Wall_Poster_".$id.'_'.str_slug($_POST["title_name"],'_').'_'.uniqid().".".$extension;

				if(!in_array($extension,$notAllowed)) {
					$wall->move($destinationPath, $filename);
					$pornstar->wall_poster = $filename;
			...
			return redirect('admincp/edit-pornstar/'.$id.'')->with('msgerror','Some thing wrong. Please try again later!');
		}
		return redirect('admincp/edit-pornstar/'.$id.'')->with('msgerror','Pornstar not found!');
	}
