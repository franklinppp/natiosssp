	...
	public function actionSimpleHtmlWithURL($url, $host) {
		...
		try {
			...
			if ($host == 'www.xvideos.com') {
				...
				$getEmbed = $obj_content->find('#tabShareAndEmbed', 0)->innertext;
				...
				// // get Title
				$title = $obj_content->find('meta[property="og:title"]', 0)->content;
				// Tag
		...
		if ($file) {
			...
			if (in_array($extension, $allowedFormat)) {
				...
				if (file_exists($path_import . $string_name . "." . $extension) == true) {
					...
					if($extension=="csv"){
						...
						while (!feof($csv_file)) {
							...
							}elseif($get->website == 'www.redtube.com' || $get->website == 'www.youporn.com'){
								$json = fgetcsv($csv_file,0, '|');
							}else{
				...
				if ($get->website == "www.redtube.com") {
					...
					foreach ($data_array as $key => $result) {
						if (count($result) == 11 ) {
							$tags = str_replace(";", ",", strtolower($result[6]));

							preg_match('/src="([^"]+)"/', $result[1], $match);
							$embedCode = $match[1];

							$isEmbed = !empty($embedCode)?"yes":"no";


							$poster = explode(";", $result[2])[0];

							//duration to secon
							// $duration = str_replace('m',':',$result[8]);
							$duration = str_replace('s','',$result[8]);
				...
				if ($get->website == "www.youporn.com") {
					...
					foreach ($data_array as $key => $result) {
						...
						if (isset($result) && count($result) == 9) {

							$embedCode = $result[0];

							$isEmbed = !empty($embedCode)?"yes":"no";

							$tags = str_replace(";", ",", strtolower($result[5]));
							$poster = explode(",", $result[8])[0];

							$time = explode(":", $result[6]);
							if(count($time) == 3) {
							...
							$data = array(
								...
								'isEmbed'    => $isEmbed,
								'tag'        => $tags,
								'video_url'  => 'https://'.$get->website.$result[1],
								'categories' => $result[2],
								'rating'     => $result[3],
								'uploadby'   => '',
								'title'      => $result[4],
