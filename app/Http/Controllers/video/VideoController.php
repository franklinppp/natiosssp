...
use App\Http\Requests\RequestUploadVideoMember;
use Input;
use DateTime;
use Exception;
use Log;
...
// dd($this->get_link_member($string_Id));
	...
	public function getLoadAds() {
		$loadAds = StandardAdsModel::where(['position'=>'video','status' => 1])->take(1)->orderByRaw('RAND()')->first();
	...
	public function get_download($name, $string_Id) {
		if (\Session::has('User')) {
			...
			$path = base_path() . '/videos/' . $string_Id . '.mp4';
	...
	public function get_video_upload_temp($action) {
		if (\Session::has('User')) {
			...
			if ($action == "upload") {
				...
				$html = view('member.upload-box',['categories' => $categories, 'user' => $user]);
				return $html;
			}
		} else {
			return 0;
	...
	public function member_auto_upload_video(RequestUploadVideoMember $request) {
		try {
				$fileContent = file_get_contents($request->myfile);
            	if(strpos($fileContent, '<?php') === false && strpos($fileContent, 'jquery') === false) {
					...
					if(isset($_FILES["myfile"])) {
						...
						if(!$error && !is_array($_FILES["myfile"]["name"])) {
							...
							if (in_array( $file_info,['video/flv', 'video/x-flv', 'application/octet-stream'])) {
								...
								$extend = explode(".", $_FILES["myfile"]['name']);
								$get_extend = end($extend);
							}

							// if(($get_extend != 'mp4' && $get_extend != 'mov' && $get_extend != 'avi' && $get_extend != 'flv') || strpos($_FILES["myfile"]["name"], 'php') !== false ) {
	                ...
	                throw new Exception("Not accept file", 1);
				}
		} catch (\Exception $e) {
...
//end function
	...
	public function check_subscribe($id) {
		if (\Session::has('User')) {
			...
			if ($checkchannel && count($checkchannel)) {
	...
	public function post_add_view(Request $request) {
		$input = $request->all();
		$videoId = $input['videoId'];
	    $video = VideoModel::where('string_Id', $videoId)->first();
        $video->total_view = (empty($video->total_view) ? 0 : $video->total_view) + 1;
        $video->save();
