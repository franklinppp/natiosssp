...
if (isset($viewvideo)) {
	...
	$isMember = \Session::has('User') ? [ 'checkPay' => SubsriptionModel::select('subscriptionId')
															->where('user_id','=',\Session::get('User')->user_id)
															->where('video_id','=',$viewvideo->string_Id)->first()
										] : NULL;
	$expriedPay = \Session::has('User') ? [
											'expriedDate' => !empty( SubsriptionModel::where('user_id','=', \Session::get('User')->user_id)
												->where('video_id','=',$viewvideo->string_Id)->first()) ?
													strtotime(
														getTimePayFortmat( SubsriptionModel::where('user_id','=', \Session::get('User')->user_id)
																->where('video_id','=',$viewvideo->string_Id)->first()->timestamp,
																SubsriptionModel::where('user_id','=',\Session::get('User')->user_id)
																->where('video_id','=',$viewvideo->string_Id)->first()->initialPeriod
														)
													) - time() : -1 ] : NULL;
	$isBuy = ($viewvideo->buy_this == '1' || checkChannelSubscribe($viewvideo->channel_Id)) ? true: false;
}
	...
	<div class="container-fluid">
		...
		<div class="row">
			<div class="col-sm-9 box-video"><!-- box video -->
				...
				<div id="video-player" class="">
					<video id="xstreamerPlayer" class="video-js vjs-default-skin" controls preload="none" data-setup='{}' style="width:100%; min-height: 550px" height="550">
						<p class="vjs-no-js">To view this video please enable JavaScript, and consider upgrading to a web browser that <a href="http://videojs.com/html5-video-support/" target="_blank">supports HTML5 video</a></p>
					</video>
				</div>
				<div class="clearfix"></div>
				<div class=" ads-under-player hidden-xs hidden-sm ">
					...
					<div class="container-fluid"><!-- container -->
						<div>
							<div class="vote-box col-xs-7 col-sm-5 col-md-3 col-lg-3">
								<div class="vote-msg">
									<div class="btn-group btn-group-lg btn-group-justified" id="vote-contain" role="group">
										<div class="btn-group btn-group-lg" role="group">
											<a href="#" role="button" class="btn btn-default left" id="vote_like_{{$viewvideo->string_Id}}">
												<i class="glyphicon glyphicon-thumbs-up"></i> <span class="hidden-xs">{{trans('home.LIKE')}}</span>
											</a>
										</div>
										<div class="btn-group btn-group-lg vote" role="group">
											<div class="text-red text-center text-bold video_rate_number">{{$percent_like['percent_like']}}%</span>(<span id="video_likes">{{$percent_like['like']}}</span>/<span id="video_dislikes">{{$percent_like['dislike']}}</span>)
											</div>
										...
										<div class="btn-group btn-group-lg" role="group">
											<a href="#" role="button" class="btn btn-default right" id="vote_dislike_{{$viewvideo->string_Id}}">
								...
								@if(\Session::has('User'))
									...
									@if ($v_setting->is_favorite == 1)
										<div id="favorite" class="pull-right m-r-5">
											<a href="javascript:void(0);" class="btn btn-default btn-radius">
												<i class="glyphicon glyphicon-heart<?= $check_favorite == 'true' ? ' pink' : '' ?>"></i>
												(<span id="change_favorited" class="hidden-xs"><?= $count_favorite; ?></span>)
											</a>
										</div>
									@endif
									@if ($v_setting->is_download == 1 && $viewvideo->website == 'upload' && ($isBuy == false || ($isBuy == true && !empty($isMember) && !empty($expriedPay) && $expriedPay['expriedDate'] >= 0)) )
								...
								@else
									@if ($v_setting->is_embed == 1)
										<div  class="pull-right m-r-5">
											<a href="javascript:void(0);" data-toggle="modal" data-target="#subscribe" class="btn btn-default btn-radius">
												<i class="glyphicon glyphicon-link"></i>
												<span class="hidden-xs">{{trans('home.EMBED')}}</span>
											</a>
										</div>
							...
							<div id="embed_video_box" class="m-t-15" style="display: none;">
								...
								<div class="form-horizontal">
									<div class="form-group">
										<label for="video_embed_code" class="col-md-2 control-label">{{trans('home.EMBED')}}</label>
										<div class="col-lg-9">
											<textarea name="video_embed_code" rows="6" id="video_embed_code" class="form-control"><iframe src="{{URL(getLang())}}/embedframe/{{$viewvideo->string_Id}}" allowfullscreen frameborder="0" width="510" height="400" scrolling="no" sandbox="allow-same-origin allow-scripts allow-popups allow-froms"></iframe></textarea>
										</div>
									</div>
									<div id="custom_size" class="form-group">
										<label for="custom_width" class="col-md-2 control-label" style="margin:0px;">{{trans('home.CUSTOM_SIZE')}}</label>
										<div class="col-lg-9">
											<div class="pull-left">
							...
							<div class="col-sm-12 col-xs-12" style="padding: 0; margin-bottom: 10px; margin-top: 10px;">
								@if(count($tag))
									<div class="m-t-10 overflow-hidden">{{trans('home.TAGS')}}:
										...
										<a class="btn btn-default btn-xs tag-wrap" role="button" href="{{URL(getLang().'search.html?keyword=')}}{{$result}}">
						...
						<div>
							<div class="m-t-10 m-b-15" style="padding-top: 10px;">
								...
								<!-- <div class="addthis_inline_share_toolbox" data-url="http://cdn.nationsporn.com/videos/{{$viewvideo->string_Id}}.mp4" data-img="{{$viewvideo->poster}}" data-title="{{$viewvideo->title_name}}"></div> -->
								<!-- Go to www.addthis.com/dashboard to customize your tools -->
							</div>
							<div class="clearfix"></div>
						</div>
				...
				<div class="box-video-related"> <!-- Box video related -->
						...
						<div class="col-xs-6 col-sm-4 col-md-3 image-left">
							<div class="col">
								<div class="col_img">
									...
									<!--    <a href="http://cdn.nationsporn.com/videos/{{$viewvideo->string_Id}}.mp4"> -->
								...
								<!-- <h3><a href="http://cdn.nationsporn.com/videos/{{$viewvideo->string_Id}}.mp4">{{$result->title_name}} </a></h3> -->
				...
				<div class="box-video-comment"><!-- Box Comment -->
					...
					<div class="container-fluid comment">
						...
						@else
						<h2 class="text-normal">{{trans('home.PLEASE_LOGIN_TO_COMMENT')}}</h2>
						@endif
						<div class="clearfix"></div>
