			...
			@if(session('msgerro'))<div class="alert alert-danger">{{session('msgerro')}}</div>@endif
       		<div class="panel panel-primary">
				...
				<form action="{{URL('admincp/email-setting')}}" class="form-horizontal" enctype="multipart/form-data" method="post" accept-charset="utf-8">
						...
						<div class="form-group">
							<div class="col-md-4"><label class="control-label">Payment Email</label></div>
							<div class="col-md-8">
								<select name="payment_email" class="form-control">
										<option>Select templete</option>
										@foreach($temp as $result)
										<option value="{{$result->id}}" <?=($email_setting->payment_email==$result->id)? 'selected="selected"' :""  ?> >{{$result->name}}</option>
										@endforeach
								</select>
							</div>
						</div>
